...
def train_and_eval(config):
    global max_budget, b
    res = b.objective_function(config, budget=max_budget)
...
def convert_to_json(history):
    ...
    for i in range(len(history)):
        ...
        if validation_regret <= inc:
            ...
            res = b.objective_function_test(architecture.arch, budget=max_budget)
...
parser.add_argument('--n_cat', default=4, type=int, help="number of categorical variables")
parser.add_argument('--n_cont', default=4, type=int, help="number of categorical variables")
...
dataset = "{}+{}".format(args.n_cat, args.n_cont)
output_path = os.path.join(args.output_path, dataset, args.folder)
