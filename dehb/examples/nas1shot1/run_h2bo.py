...
# https://github.com/automl/nasbench-1shot1/blob/master/optimizers/bohb/run_bohb.py
...
for space in spaces:
    ...
    history = h2bo.run(args.n_iters, min_n_workers=num_workers)

    h2bo.shutdown(shutdown_workers=True)
    NS.shutdown()

    fh = open(os.path.join(output_path,
                           'algo_{}_{}_ssp_{}_seed_{}.obj'.format('H2BO',
                                                                  args.run_id,
                                                                  space,
                                                                  args.seed)), 'wb')
