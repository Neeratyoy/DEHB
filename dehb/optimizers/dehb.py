...
class DEHB_1(DEHBBase):
    '''DEHB with random mutation pop for lowest budget
    '''
    def __init__(self, async_strategy='orig', **kwargs):
        super().__init__(**kwargs)
        self.max_age = np.inf
        ...
        for iteration in range(iterations):
            ...
            if iteration == 0:  # first HB bracket, first iteration
                ...
                for i_sh in range(num_SH_iters):
                    ...
                    if iteration % self.max_SH_iter == 0 and i_sh == 0:
                        pop_len = int(self.de[budget].pop_size / (self.eta ** beta_param))
                        if pop_len < self.de[budget]._min_pop_size:
                            alt_population = None
                        else:
                            alt_population = self.init_population(pop_size=pop_len)
                            if debug:
                                print("Creating random population for mutation of "
                                      "size: {}".format(alt_population.shape))
                            beta_param += 1
                    '''Addition ends here'''
                    ...
                    if i_sh < (num_SH_iters - 1):  # when not final SH iteration
                        ...
                        if self.async_strategy in ['orig', 'basic'] and \
                            ...
                            # reordering to have the top individuals in front
                            # rank_include = np.sort(np.argsort(self.de[budget].fitness)[:pop_size])
                            rank_include = np.sort(np.random.choice(np.arange(len(self.de[budget].population)), size=pop_size))
                            rank_exclude = list(set(np.arange(len(self.de[budget].population))) - \
                                                set(rank_include))
                            self.de[budget].population = \
                                np.concatenate((self.de[budget].population[rank_include],
...
class DEHB_2(DEHBBase):
    '''DEHB with max_pop_size determined by addition in 2st HB bracket
    '''
    def __init__(self, async_strategy='orig', **kwargs):
        super().__init__(**kwargs)
        self.max_age = np.inf
